...
from utils.utils import early_stopping, dict2str, TrainState
...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model):
        ...
        # svd
        self.train_state = TrainState.normal
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None):
        ...
        total_loss = None
        loss_batches = []
        # switch to dual flow state
        if epoch_idx == self.config['break_point']:
            self.model.init_ego_augment_and_flaw()
            self.train_state = TrainState.dual_flow
        for batch_idx, interaction in enumerate(train_data):
            self.optimizer.zero_grad()
            losses = loss_func(interaction, self.train_state) # interaction size: (3, 2048)
